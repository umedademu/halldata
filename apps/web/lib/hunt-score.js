...
const BEAM_HIKARI_STORE_COMMON_SCORE_LINEAR_RAW_MAX = 145;
...
const ESPACE_SHINKOIWA_NORTH_TARGET_MACHINES = [
  { name: "ネオアイムジャグラーEX", aliases: ["ネオアイムジャグラーＥＸ"] },
];
...
const HUNT_SCORE_STORE_CONFIGS = [
  ...
  },
  {
    ...
    targetMachines: ESPACE_SHINKOIWA_NORTH_TARGET_MACHINES,
    defaultLogicKey: "apark",
    machineHighContentRules: {
      "ネオアイムジャグラーEX": "espace-shinkoiwa-north-neo-aim",
      "ネオアイムジャグラーＥＸ": "espace-shinkoiwa-north-neo-aim",
    },
  },
  {
...
const MISTER_JUGGLER_BONUS_SETTING_RATES = [
  { setting: 1, bb: 1 / 268.6, rb: 1 / 374.5 },
  { setting: 2, bb: 1 / 267.5, rb: 1 / 354.2 },
  { setting: 3, bb: 1 / 260.1, rb: 1 / 331.0 },
  { setting: 4, bb: 1 / 249.2, rb: 1 / 291.3 },
  { setting: 5, bb: 1 / 240.9, rb: 1 / 257.0 },
...
function calculateNeoAimSettingFivePlusProbability(row) {
  ...
  if (cacheKey && typeof cacheKey === "object" && NEO_AIM_SETTING_FIVE_PLUS_PROBABILITY_CACHE.has(cacheKey)) {
    return NEO_AIM_SETTING_FIVE_PLUS_PROBABILITY_CACHE.get(cacheKey);
  }
  ...
  const cacheAndReturn = (value) => {
    if (cacheKey && typeof cacheKey === "object") {
      NEO_AIM_SETTING_FIVE_PLUS_PROBABILITY_CACHE.set(cacheKey, value);
    }
    return value;
  };
...
function calculateMisterJugglerSettingProbabilities(row) {
  const cacheKey = row?.row && typeof row.row === "object" ? row.row : row;
  if (cacheKey && typeof cacheKey === "object" && MISTER_JUGGLER_SETTING_PROBABILITY_CACHE.has(cacheKey)) {
    return MISTER_JUGGLER_SETTING_PROBABILITY_CACHE.get(cacheKey);
  }
  const games = Math.round(readWindowField(row, "games"));
  const bbCount = Math.round(readWindowField(row, "bbCount"));
  const rbCount = Math.round(readWindowField(row, "rbCount"));
  ...
  const cacheAndReturn = (value) => {
    if (cacheKey && typeof cacheKey === "object") {
      MISTER_JUGGLER_SETTING_PROBABILITY_CACHE.set(cacheKey, value);
    }
    return value;
  };
  ...
  if (games === 0) {
    return cacheAndReturn({
      p4plus: 3 / MISTER_JUGGLER_BONUS_SETTING_RATES.length,
      p5plus: 2 / MISTER_JUGGLER_BONUS_SETTING_RATES.length,
      p3minus: 3 / MISTER_JUGGLER_BONUS_SETTING_RATES.length,
    });
  }

  const logRows = MISTER_JUGGLER_BONUS_SETTING_RATES.map((rate) => ({
    setting: rate.setting,
    logValue:
      calculateLogBinomialProbabilityForHuntScore(bbCount, games, rate.bb) +
      calculateLogBinomialProbabilityForHuntScore(rbCount, games, rate.rb),
  ...
  return cacheAndReturn({
    ...
    p3minus:
      weightedRows
        .filter((rowValue) => rowValue.setting <= 3)
        .reduce((total, rowValue) => total + rowValue.weight, 0) / totalWeight,
  });
}
...
function isWonderlandMinamigaokaMisterGoodContentWindowRow(row) {
  ...
  const combinedDenominator = calculateCombinedDenominatorFromWindowRow(row);
  const rbDenominator = calculateRbDenominatorFromWindowRow(row);
  const probabilities = calculateMisterJugglerSettingProbabilities(row);
  const p4plus = probabilities?.p4plus;
  return (
    games >= 3000 &&
    ((combinedDenominator <= 150 && rbDenominator <= 360) ||
      (Number.isFinite(p4plus) && p4plus >= 0.35))
  );
}

function isWonderlandMinamigaokaMisterHighContentWindowRow(row) {
  const games = readWindowField(row, "games");
  const combinedDenominator = calculateCombinedDenominatorFromWindowRow(row);
  const rbDenominator = calculateRbDenominatorFromWindowRow(row);
  const probabilities = calculateMisterJugglerSettingProbabilities(row);
  ...
  return (
    games >= 3500 &&
    ((combinedDenominator <= 140 && rbDenominator <= 320) ||
      (Number.isFinite(p4plus) && p4plus >= 0.5))
  );
}

function isWonderlandMinamigaokaMisterStrongHighContentWindowRow(row) {
  const games = readWindowField(row, "games");
  const combinedDenominator = calculateCombinedDenominatorFromWindowRow(row);
  const rbDenominator = calculateRbDenominatorFromWindowRow(row);
  const probabilities = calculateMisterJugglerSettingProbabilities(row);
...
function isMachineHighContentWindowRow(row, machineName, config = null) {
  ...
  if (
    ...
    if (contentRule === "park-kitasenju-neo-aim") {
      ...
      return games >= 5000 && rbDenominator <= 300 && combinedDenominator <= 145;
    }
    ...
    if (contentRule === "iidabashi-presas-neo-aim") {
      ...
      return (
        games >= 4500 &&
        ((Number.isFinite(settingFivePlusProbability) && settingFivePlusProbability >= 0.5) ||
          (rbDenominator <= 285 && combinedDenominator <= 140))
      );
    }
  ...
  if (normalizedMachineName === normalizeText("ミスタージャグラー")) {
    if (contentRule === "wonderland-minamigaoka-mister") {
      return isWonderlandMinamigaokaMisterHighContentWindowRow(row);
    }
...
function isMachineGoodContentWindowRow(row, machineName, config = null) {
  ...
  if (normalizedMachineName === normalizeText("ネオアイムジャグラーEX")) {
    ...
    if (contentRule === "espace-shinkoiwa-north-neo-aim") {
      ...
      return games >= 2500 && rbDenominator <= 330 && combinedDenominator <= 155;
    }
    if (contentRule === "messe-minamisenju-neo-aim") {
      const settingFivePlusProbability = calculateNeoAimSettingFivePlusProbability(row);
      if (Number.isFinite(settingFivePlusProbability)) {
    ...
    if (contentRule === "park-takenotsuka-studio-neo-aim") {
      ...
      return (
        ...
        ((Number.isFinite(settingFivePlusProbability) && settingFivePlusProbability >= 0.3) ||
          (rbDenominator <= 315 && combinedDenominator <= 150))
      );
    }
    if (contentRule === "park-kitasenju-neo-aim") {
      const settingFivePlusProbability = calculateNeoAimSettingFivePlusProbability(row);
      ...
      return games >= 4000 && rbDenominator <= 330 && combinedDenominator <= 150;
    }
    if (contentRule === "park-kitasenju-sss-neo-aim") {
      const settingFivePlusProbability = calculateNeoAimSettingFivePlusProbability(row);
      if (Number.isFinite(settingFivePlusProbability)) {
    ...
    if (contentRule === "mitoya-jackpot-kinshicho-neo-aim") {
      return games >= 1500 && rbDenominator <= 300;
    }
    if (contentRule === "mitoya-asakusa-senzoku-neo-aim") {
      return games >= 2500 && rbDenominator <= 300 && combinedDenominator <= 145;
    }
    ...
    if (contentRule === "kintoki-kamata-neo-aim") {
      ...
      if (Number.isFinite(settingFivePlusProbability)) {
        return (
          games >= 2500 &&
          (settingFivePlusProbability >= 0.3 ||
            (rbDenominator <= 315 && combinedDenominator <= 145))
        );
      }
      return games >= 2500 && rbDenominator <= 315 && combinedDenominator <= 145;
    }
    if (contentRule === "iidabashi-presas-neo-aim") {
      const settingFivePlusProbability = calculateNeoAimSettingFivePlusProbability(row);
      return (
  ...
  if (normalizedMachineName === normalizeText("ミスタージャグラー")) {
    if (contentRule === "wonderland-minamigaoka-mister") {
      return isWonderlandMinamigaokaMisterGoodContentWindowRow(row);
    }
  }
  if (
    (normalizedMachineName === normalizeText("スマスロ ミリオンゴッド") ||
...
function isMachineStrongHighContentWindowRow(row, machineName, config = null) {
  ...
  if (normalizedMachineName === normalizeText("ミスタージャグラー")) {
    if (contentRule === "wonderland-minamigaoka-mister") {
      return isWonderlandMinamigaokaMisterStrongHighContentWindowRow(row);
    }
    const rbCount = readWindowField(row, "rbCount");
  ...
  if (
    ...
    return games >= 5000 && rbDenominator <= 270 && combinedDenominator <= 135;
  }
  if (
    normalizedMachineName === normalizeText("ネオアイムジャグラーEX") &&
  ...
  if (
    normalizedMachineName === normalizeText("ネオアイムジャグラーEX") &&
    readMachineContentRule(config, machineName) === "espace-shinkoiwa-north-neo-aim"
  ) {
    const settingFivePlusProbability = calculateNeoAimSettingFivePlusProbability(row);
    if (Number.isFinite(settingFivePlusProbability)) {
      return games >= 4000 && settingFivePlusProbability >= 0.7;
  ...
  if (
    ...
    return (
      games >= 5000 &&
      ((Number.isFinite(settingFivePlusProbability) && settingFivePlusProbability >= 0.7) ||
        (rbDenominator <= 270 && combinedDenominator <= 132))
    );
  }
  if (
    normalizedMachineName === normalizeText("ネオアイムジャグラーEX") &&
...
function calculateWindowMetrics(
  ...
  const recentFiveLowGames1000Count = recentFiveRows.filter((windowRow) => readWindowField(windowRow, "games") < 1000).length;
  ...
  const recentFourBonusTotal = sumWindowField(recentFourRows, "bbCount") + sumWindowField(recentFourRows, "rbCount");
  ...
  const recentFourRbTotal = sumWindowField(recentFourRows, "rbCount");
  ...
  const previousMachineMyJugglerProbabilities =
    ...
    currentMachineNameNormalized === normalizeText("マイジャグラー")
      ...
      : null;
  const previousMachineMisterJugglerProbabilities =
    currentMachineNameNormalized === normalizeText("ミスタージャグラー")
      ? calculateMisterJugglerSettingProbabilities({
          row,
          differenceValue: readHuntScoreDifferenceValue(row, config.differenceMode, currentMachineName),
        })
      : null;
  ...
  const previousAdjacentMachineSettingFivePlusProbabilityAverage = (() => {
    const values = listAdjacentSameMachineRowsByOrder(
      currentDateRows,
      row,
      config,
      currentMachineName,
      ...
      .map((dateRow) => calculateNeoAimSettingFivePlusProbability({ row: dateRow }))
      .filter((value) => Number.isFinite(value));
  ...
  const recentFiveBigLoss1000Count = recentFiveRows.filter(
  ...
  const recentTwentyOneMinDifference =
    recentTwentyOneRows.length > 0
      ...
      : 0;
  const recentTwentyEightMinDifference =
    recentTwentyEightRows.length > 0
      ? Math.min(...recentTwentyEightRows.map((windowRow) => readNumber(windowRow?.differenceValue)))
      : 0;
  ...
  return {
    ...
    previousAdjacentMachineSettingFivePlusProbabilityAverage,
