...
def backfill_record(machine_name: str, record: dict[str, Any]) -> tuple[bool, bool]:
    before_values = {field_name: record.get(field_name) for field_name in ESTIMATED_GRAPE_FIELD_NAMES}
    for field_name in ESTIMATED_GRAPE_FIELD_NAMES:
        record.pop(field_name, None)

    data_source = str(record.get("data_source") or "").strip()
    add_estimated_grape_fields(record, machine_name, data_source)
    updated = record.get("estimated_grape_denominator") not in (None, "")
    after_values = {field_name: record.get(field_name) for field_name in ESTIMATED_GRAPE_FIELD_NAMES}
    changed = before_values != after_values
    return updated, changed


def backfill_machine_payload(machine_payload: dict[str, Any]) -> tuple[int, int, int, int, bool]:
...
def make_default_backup_dir(root_dir: Path) -> Path:
    ...
    return root_dir / "local_data" / "backups" / f"estimated-grape-values-{timestamp}"
