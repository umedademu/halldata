...
def backfill_machine_payload(machine_payload: dict[str, Any], *, recalculate: bool = False) -> tuple[int, int, int, int, bool]:
    ...
    for record in records:
        if not isinstance(record, dict):
            continue
        record_count += 1
        before_value = record.get("bonus_difference_value")
        if record_has_bonus_difference(record) and not recalculate:
            continue
        ...
        updated_count += 1
        if before_value != bonus_difference_value:
            changed_count += 1
...
def make_default_backup_dir(root_dir: Path) -> Path:
    ...
    return root_dir / "local_data" / "backups" / f"bonus-difference-values-{timestamp}"
...
def backfill_machine_file(
    ...
    *,
    ...
    record_count, updated_count, skipped_count, changed_count, changed = backfill_machine_payload(
        machine_payload,
...
def main() -> int:
    ...
    parser.add_argument("--recalculate", action="store_true", help="保存済みのボーナス数基準差枚も再計算します")
    ...
    print(
        ...
        f"確認行{summary.record_count}件、更新行{summary.updated_record_count}件、"
        f"変更行{summary.changed_record_count}件、"
